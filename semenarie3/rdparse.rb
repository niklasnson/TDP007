...
class Logic
  ...
  def initialize
    @myparser = Parser.new("Logicly++") do
      token(/\s+/)          # skip empty
      token(/\w+/) {|m| m } # get words
      token(/\d+/){ |m| m.to_i }
      token(/./) { |m| m }  # (, ) 
  ...
  def roll!
    begin 
      ...
      puts "*"*80
      printf("%-50s\n", "shame on you! no rick roll for you!") 
      puts "*"*80
    end
  end
  ...
  def log(state = true)
    if state
      @myparser.logger.level = Logger::DEBUG
    else
      @myparser.logger.level = Logger::WARN
    end
  end
end

#- uncomment to run without test.
